  ...
  return (
    <div className="mt-6 p-3 space-y-10 bg-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 bg-clip-text text-transparent">
          📘 Generated Notes
        </h2>
        ...
        <section>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <ReactMarkDown components={markDownComponent}>
              {formatNotesToMarkdown(result.notes)}
            </ReactMarkDown>
          </div>
        </section>
      )}
      ...
      {quickRevision && (
        <section className="rounded-xl bg-gradient-to-r font-green-100 to-green-50 border border-green-200 p-6">
      ...
      {result.diagram?.data && (
        <section>
          ...
          <p className="mt-3 text-xs text-gray-500 italic">
            ℹ If you need this diagram for future reference or revision, you can
      ...
      {result.charts && result.charts.length === 0 && (
        <p className="text-sm text-gray-400 italic">
         📈 Charts arr not revelant for this topic.
        </p>
      )}
...
const formatNotesToMarkdown = (text) => {
  return text
    ...
    .map((line) => {
      ...
      // Heading detect (HTML:, CSS:, JavaScript:)
      if (/^[A-Z][A-Z\s]+:$/.test(trimmed)) {
        return `## ${trimmed.replace(":", "")}`;
      }

      // sub bullets already fine
      return line;
    })
    .join("\n");
};
